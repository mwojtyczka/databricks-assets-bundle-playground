...
TEST_CLUSTER_ID = os.getenv('DATABRICKS_TEST_CLUSTER_ID')
...
def test_main():
    ...
    ctx = w.command_execution.create(cluster_id=TEST_CLUSTER_ID, language=Language.SQL).result()
    command = "SELECT * FROM samples.nyctaxi.trips LIMIT 5;"
    results = w.command_execution.execute(cluster_id=TEST_CLUSTER_ID, command=command, context_id=ctx.id, language=Language.SQL).result()
